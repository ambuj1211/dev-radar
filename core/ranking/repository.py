...
def freshness_score(updated_at: str, max_age_days: int = 180) -> float:
...
def popularity_score(stars: int, forks: int) -> float:
    ...
    stars_score = min(math.log10(max(stars, 1)) / 5 * 100, 100)
    forks_score = min(math.log10(max(forks, 1)) / 4 * 100, 100)
...
def community_score(
    ...
    issues_score = min(math.log10(max(open_issues, 1)) / 4 * 100, 100)
    contributor_score = min(
        math.log10(max(contributors, 1)) / 3 * 100,
        100,
    )

    return (issues_score * 0.3) + (contributor_score * 0.7)


def calculate_radar_score(repository: dict) -> float:
...
def rank_repositories(repositories: list[dict]) -> list[dict]:
    """Add radar scores and return repositories ranked by score."""
    ...
    for repository in repositories:
        ...
        repository["radar_score"] = calculate_radar_score(
            repository
        )
